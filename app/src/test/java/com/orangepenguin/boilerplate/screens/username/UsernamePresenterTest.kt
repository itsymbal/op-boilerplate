...
class UsernamePresenterTest {
    ...
    fun shouldPopulateUsernameAndRememberCheckboxIfUsernamePreferenceSet() {
        `when`(sharedPreferencesUtil.getPreference(Constants.PREF_USERNAME, null)).thenReturn(USERNAME)

        usernamePresenter.takeView(mockView)

        verify<SharedPreferencesUtil>(sharedPreferencesUtil).getPreference(Constants.PREF_USERNAME, null)
